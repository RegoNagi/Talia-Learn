...
describe('calculateQuizScore', () => {
  ...
  it('يقبل الإجابة الرقمية جوه هامش السماحية ويرفض برّاها', () => {
    ...
    expect(calculateQuizScore(questions, { q1: '101' }).score).toBe(10); // جوه الهامش
    expect(calculateQuizScore(questions, { q1: '98' }).score).toBe(10); // على حافة الهامش
    expect(calculateQuizScore(questions, { q1: '95' }).score).toBe(0); // برّه الهامش
    expect(calculateQuizScore(questions, { q1: 'مش رقم' }).score).toBe(0); // مدخل غير صالح
  });
